...
def load_batch_config(key_params):
    ...
    if not param['deep_model']:
        ...
        if key_params['experiment_mode'].endswith('discrete_feature_selection'):
            param['experiment_name'] = '.'.join([param['timemark'], key_params['experiment_mode'], 'C=16', 'feature_number=%s' % key_params['k_feature_to_keep'], 'context=%s' % param['context_set'], 'feature=%s' % param['feature_set'], 'similarity=true' if param['similarity_feature'] else 'similarity=false'])
