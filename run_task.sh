...
do
    ...
    for ((i=START;i<=END;i++)); do
        ...
        sbatch --export=CONTEXT=$CONTEXT,i=$i,PYTHONPATH=/ihome/pbrusilosky/rum20/.conda/envs/py35/bin/python --job-name=context-$CONTEXT.feature-$i.run --output=slurm_log/context-$CONTEXT.feature-$i.run_task_log.out run_task.sbatch;
    done
done
